...
from config import moc_path, cda_output_path, csc_output_path
import logging

# Configure logger
logging.basicConfig(filename='info.log', encoding='utf-8', format='%(asctime)s - %(levelname)s - %(message)s', datefmt='%m/%d/%Y %I:%M:%S %p', level=logging.DEBUG)
...
def run(archive='CDA', classifier='lightcurve', days_ago=2.0, novel_objects=True):
    try:
        ...
        logging.info(f'######## ARCHIVE={archive} CLASSIFIER={classifier} ########')
